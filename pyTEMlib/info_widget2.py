...
class InfoWidget(EELSBaseWidget):
    def __init__(self, datasets=None):
    ...
    def set_flux(self, value):
        ...
        if self.info_tab[9, 0].value == 'None':
            ...
            self.datasets['_relationship']['low_loss'] = ll_key
            ...
            if self.datasets[ll_key].metadata['experiment']['exposure_time'] == 0.0:
                if self.datasets[ll_key].metadata['experiment']['single_exposure_time'] == 0.0:
                    return
                else:
                    self.datasets[ll_key].metadata['experiment']['exposure_time'] = (self.datasets[ll_key].metadata['experiment']['single_exposure_time'] *
                                                                                     self.datasets[ll_key].metadata['experiment']['number_of_frames'])

            self.datasets[self.key].metadata['experiment']['flux_ppm'] = ((np.array(self.datasets[ll_key])*1e-6).sum() /
                                                                          self.datasets[ll_key].metadata['experiment']['exposure_time'] /
                                                                          number_of_pixels)
            self.datasets[self.key].metadata['experiment']['flux_ppm'] *= self.datasets[self.key].metadata['experiment']['exposure_time']
            if 'SPECT' in self.datasets[ll_key].data_type.name:
    ...
    def update_sidebar(self):
        ...
        offset = self.datasets[self.key].energy_loss[0]
        dispersion = self.datasets[self.key].energy_loss[1] - offset
        ...
        ll_key = 'None'
        if '_relationship' in self.datasets:
            if 'low_loss' in self.datasets['_relationship']:
                ll_key = self.datasets['_relationship']['low_loss']  
                ll_key = f'{ll_key}: {self.datasets[ll_key].title}'
...
class LowLossWidget(EELSBaseWidget):
    def __init__(self, datasets, sidebar=None):
        if sidebar is None:
    ...
    def update_sidebar(self):
        ...
        self.low_loss_tab[9, 0].options = reference_list
        
        if 'SPECTRUM' in self.dataset.data_type.name:
            for i in range(14, 17):
                self.low_loss_tab[i, 0].layout.display = "none"
        else:
            for i in range(14, 17):
                self.low_loss_tab[i, 0].layout.display = "flex"
    ...
    def set_action(self):
        self.low_loss_tab[0, 0].observe(self.update_dataset)
        ...
        self.low_loss_tab[4, 0].on_click(self.get_resolution_function)
        self.low_loss_tab[6, 2].observe(self.set_y_scale, names='value')
        self.low_loss_tab[6, 0].observe(self._update, names='value')
